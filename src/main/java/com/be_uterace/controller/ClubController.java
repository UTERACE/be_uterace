...
public class ClubController {
    ...
    public ResponseEntity<ClubPaginationResponse> manageClubCreatedController(
            @RequestParam int current_page,
            @RequestParam int per_page, @RequestParam String search_name, Authentication authentication) {
        ClubPaginationResponse clubPaginationResponse = clubService.getManageClubCreated(
                current_page, per_page, search_name, authentication);
        return ResponseEntity.ok(clubPaginationResponse);
    }
    ...
    @GetMapping(value = {"/member/{club_id}"})
    public ResponseEntity<List<UserFindResponse>> findUserByClubIdController(
            @PathVariable Integer club_id,
            @RequestParam String search) {
        List<UserFindResponse> user = clubService.findUserByClubId(club_id, search);
